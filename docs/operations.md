...
`meridian-mesh-failure-forge` is centered on distributed systems work in SQL. The technical target is: Implement an SQL distributed systems project for failure incremental indexing, using append-only fixtures and checkpoint recovery checks.
...
The score is deterministic. The current threshold is 179. Risk has a penalty of 5, latency has a penalty of 4, and weight contributes a bonus of 3.
